...
StorageTransferOperator::StorageTransferOperator(MscBulkOnlyHandler* handler) :
    handler_(handler) {
}

void* StorageTransferOperator::alloc_transfer_handle(std::size_t buffer_length, int,
                                                      const UsbIpHeaderBasic& header,
                                                      const SetupPacket&) {
    auto* trx = new StorageIoTransfer{};
    SPDLOG_DEBUG("STO::alloc handle={:p} dir={} len={}",
                 static_cast<const void*>(trx),
                 header.direction == UsbIpDirection::In ? "IN" : "OUT",
                 buffer_length);
...
void StorageTransferOperator::free_transfer_handle(void* handle) {
    ...
    delete StorageIoTransfer::from_handle(handle);
}
...
void StorageTransferOperator::send_transfer_data(void* handle, asio::ip::tcp::socket& sock,
    ...
    SPDLOG_DEBUG("STO::send handle={:p} len={} direct_io={} lba={} offset={} ext_buf={:p}",
                 static_cast<const void*>(handle), length, trx->direct_io,
                 trx->file_lba, trx->file_offset,
                 static_cast<const void*>(trx->external_buf));

    // 仅 mmap READ 走零拷贝 sendfile/TransmitFile，CSW 等 fallback 数据不碰文件
    if (trx->direct_io && backend && backend->send_direct(trx->file_lba, trx->file_offset, length,
                                                           static_cast<intptr_t>(sock.native_handle()), ec)) {
...
void StorageTransferOperator::recv_transfer_data(void* handle, asio::ip::tcp::socket& sock,
                                                  std::size_t length, std::error_code& ec) {
    auto* trx = StorageIoTransfer::from_handle(handle);
    auto* backend = handler_->get_backend();
    ...
    if (trx->direct_io && trx->external_buf && backend &&
        backend->recv_direct(trx->file_lba, trx->file_offset, length,
                             static_cast<intptr_t>(sock.native_handle()), ec)) {
