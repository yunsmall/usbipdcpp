...
namespace usbipdcpp
{
                                    ...
                                    std::function<void(int, int)> callback)
    {
        ...
        move_raw(from_hid_x, from_hid_y, to_hid_x, to_hid_y, duration_ms,
                 [this, callback](std::int16_t x, std::int16_t y)
                 {
                     if (callback)
                     {
                                              ...
                                              std::function<void(int, int)> callback)
    {
        ...
        {
            double u = 1.0 - t;
            double x = u * u * u * from_hid_x + 3 * u * u * t * ctrl1_x + 3 * u * t * t * ctrl2_x + t * t * t *
                to_hid_x;
            double y = u * u * u * from_hid_y + 3 * u * u * t * ctrl1_y + 3 * u * t * t * ctrl2_y + t * t * t *
                to_hid_y;
            return {x, y};
        };
        ...
        while (t < 1.0 && !should_stop_)
        {
            ...
            std::int16_t x = static_cast<std::int16_t>(std::clamp(static_cast<int>(std::round(final_x)), 0,
                                                                  static_cast<int>(HID_MAX)));
            std::int16_t y = static_cast<std::int16_t>(std::clamp(static_cast<int>(std::round(final_y)), 0,
                                                                  static_cast<int>(HID_MAX)));
