...
public:
    ...
    virtual void* get_direct_buffer(std::uint64_t lba) { return nullptr; }

    // 零拷贝发送（sendfile / TransmitFile），默认 false → 回退 asio::write
    virtual bool send_direct(std::uint64_t lba, std::size_t offset, std::size_t length,
                             intptr_t sock_fd, std::error_code& ec) {
        return false;
    }

    // 零拷贝接收（splice sock→pipe→file），默认 false → 回退 asio::read
    virtual bool recv_direct(std::uint64_t lba, std::size_t offset, std::size_t length,
                             intptr_t sock_fd, std::error_code& ec) {
        return false;
    }
