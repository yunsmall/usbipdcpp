 ...
 * - try_pull_data(length)：可选 pull 模型，默认返回空
...
public:
    ...
    void on_in_request(std::uint8_t ep, std::uint32_t seqnum, std::uint32_t length, TransferHandle transfer) {
        ...
        if (endpoint_requests.empty(ep)) {
            ...
            if (!pulled.empty()) {
                // 整包入缓冲后由本次请求消费（超出请求长度的部分留在缓冲，
                // 由 try_send_pending_locked 匹配其他挂起请求）
                self().push_locked(std::move(pulled));
                self().try_send_one(ep, seqnum, length, std::move(transfer));
...
public:
    ...
    void try_send_one(std::uint8_t ep, std::uint32_t seqnum, std::uint32_t length, TransferHandle transfer) {
        ...
        trx->data.assign(front.begin(), front.begin() + send_len);
...
public:
        ...
        while (offset < size) {
            // 阶段1：等待缓冲有空间
            {
                ...
                if (buffer.available() == 0) {
                    ...
                    while (buffer.available() == 0 && !disconnected) {
                        if (timeout_ms == 0) {
                            space_cv.wait(lock);
                        }
                        else {
                            if (space_cv.wait_for(lock, std::chrono::milliseconds(timeout_ms)) ==
    ...
    void try_send_one(std::uint8_t ep, std::uint32_t seqnum, std::uint32_t length, TransferHandle transfer) {
        ...
        buffer.read(trx->data.data(), send_len);
        trx->actual_length = send_len;
        // 缓冲腾出空间，唤醒阻塞等待的写者
        space_cv.notify_one();
        session->submit_ret_submit(UsbIpResponse::UsbIpRetSubmit::create_ret_submit_ok_with_no_iso(
